...
router.post("/customer", async function (req, res) {
  ...
  if (duplicate.rows.length !== 0) {
    ...
    await client.query(addingUserInfoSQL, [firstname, secondname, addressIDGen]);
    //
    ...
    await client.query(addingUsersSQL, [username, passwordEncrypted, email, customer_id]);
...
router.post("/verify", async function (req, res) {
  ...
  if (hash.rows[0]) {
    ...
    res.json(
      {
        status: "Username does not exist. Please try again or register an account",
      },
      400
    );
  }
...
router.post("/restaurant", async function (req, res) {
  ...
  if (duplicate.rows.length !== 0) {
    ...
    await client.query(addingRestaurantSQL, [username, passwordEncrypted, email, restaurant_id]);
    ...
    if (checkUser.rows.length > 0) {
      if (checkUser.rows[0].restaurant_id) {
        const accountDetails = await client.query(
          "SELECT * FROM users JOIN restaurants ON users.restaurant_id  = restaurants.id JOIN addresses ON restaurants.address_id = addresses.uuid WHERE users.id = $1 ",
          [id]
        );
