...
router.post("/", async function (req, res) {
  ...
  const userKey = await client.query(`SELECT id FROM users WHERE email=$1`, [email]);

  try {
    await client.query("INSERT INTO sessions (uuid, user_id) VALUES ($1, $2)", [sessionID, userKey.rows[0].id]);
