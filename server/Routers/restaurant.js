...
router.get("/details", async function (req, res) {
  const client = await pool.connect();
  try {
    const restaurantDetails = await client.query("SELECT * FROM restaurants WHERE current_slots > 0");
...
router.get("/:id/orders", async function (req, res) {
  ...
  try {
    const allRestaurantOrders = await client.query(
      "SELECT * FROM orders JOIN customers ON orders.customer_id = customers.id WHERE orders.restaurant_id = $1 ",
      [id]
    );
