...
router.get("/", async function (req, res) {
	...
	try {
		const allRestaurantData = await client.query(
			"SELECT * FROM restaurants JOIN addresses ON restaurants.address_id = addresses.uuid"
		);
...
router.get("/:id/orders/today", async function (req, res) {
	...
	try {
		await client.query(
			"SELECT * FROM orders WHERE restaurant_id = $1 AND (CURRENT_TIMESTAMP::date = created_at::date)",
			[id]
		);
		res
			.status(200)
